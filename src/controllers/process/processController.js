...
export const startProcess = async (req, res) => {
  try {
    ...
    switch (processType) {
      case "work":
        operationResult = await startWork(req.userId)
        break
      case "training":
        operationResult = await startTraining(req.userId)
        break
      case "sleep":
        operationResult = await startSleep(req.userId)
        break
      case "skill":
        ...
        operationResult = await buySkill(req.userId, skillId, sub_type)
        break
      case "food":
        const foodId = parseInt(req.query.typeId)
        if (!foodId) throw new Error("Incorrect foodId!")
        operationResult = await buyFood(req.userId, foodId)
        break
    }
...
export const stopActiveProcess = async (req, res) => {
  ...
  try {
    const result = await withTransaction(async (session) => {
      ...
      const activeProcess = await processModel
        .findOne({ id: req.userId, active: true })
        .session(session)
      if (!activeProcess) throw new Error("Active Process not found!")

      await processModel
        .deleteOne({ id: req.userId, active: true })
        .session(session)
      return { status: "ok" }
    })
    ...
    res
      .status(e.message === "User not found!" ? 404 : 404)
      .json({ error: e.message })
  }
}
  ...
  try {
    ...
    switch (activeProcess.type) {
      ...
      case "sleep":
        ...
        break
      case "skill":
        const skillProcess = await processModel.findOne({
          type: "skill",
          sub_type: subType,
        })
      ...
      case "food":
        if (
          moment().diff(moment(activeProcess.createdAt), "seconds") >=
          (activeProcess.target_duration_in_seconds ||
            activeProcess.base_duration_in_seconds)
        ) {
          await withTransaction(async (session) => {
            await processModel
              .deleteOne({ _id: activeProcess._id })
              .session(session)
          })
        }
    ...
    return res
      ...
      .json(err.data || { error: "Internal server error" })
  }
}
